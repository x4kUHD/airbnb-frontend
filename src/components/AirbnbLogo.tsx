...
/** The Airbnb "Bélo" mark on its own. */
export function BeloMark({ className = "h-8 w-8" }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
...
/**
 * Bélo + wordmark. The wordmark hides below `lg`, exactly as on airbnb.com.
 */
export default function AirbnbLogo({
  ...
  return (
    <span className={`flex items-center gap-[3px] text-rausch ${className}`}>
      <BeloMark className="h-8 w-8" />
      {wordmark && (
        <span className="hidden text-[22px] font-bold leading-none tracking-[-0.045em] lg:inline">
          airbnb
        </span>
      )}
    </span>
